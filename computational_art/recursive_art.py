...
def build_random_function(min_depth, max_depth):
    ...
    if min_depth == 1:
        return random.choice(["x","y"]) #randomly returns x or y
    else:
        if chosenFunction=="prod" or "avg":
...
def evaluate_random_function(f, x, y):
    ...
    if f[0] == 'x': 
        ...
        return sin(2*pi*evaluate_random_function(f[1],x,y))
    elif f[0] == 'prod':
...
def generate_art(filename, x_size=350, y_size=350):
    ...
    red_function = build_random_function(1, 2)
    green_function = build_random_function(4, 6)
    blue_function = build_random_function(2, 2)
